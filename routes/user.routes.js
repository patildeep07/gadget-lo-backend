...
userRouter.delete(
  "/:userId/remove-from-wishlist/:productId",
  async (req, res) => {
    try {
      const updatedUser = await deleteFromWishlist(
        req.params.userId,
        req.params.productId,
      );
...
userRouter.delete("/:userId/remove-from-cart/:productId", async (req, res) => {
  try {
    const updatedUser = await deleteFromCart(
      req.params.userId,
      req.params.productId,
    );
...
userRouter.post(
  "/:userId/wishlist-product-quantity/:productId",
  async (req, res) => {
    try {
      const { quantity } = req.body;

      const updatedUser = await modifyWishlistProductQuantity(
        req.params.userId,
        req.params.productId,
        quantity,
      );
...
userRouter.post(
  "/:userId/cart-product-quantity/:productId",
  async (req, res) => {
    try {
      const { quantity } = req.body;

      const updatedUser = await modifyCartProductQuantity(
        req.params.userId,
        req.params.productId,
        quantity,
      );
...
userRouter.post("/:userId/place-order", async (req, res) => {
  try {
    const updatedUser = await emptyCart(req.params.userId);
